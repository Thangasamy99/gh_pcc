...
import { ImagingPaymentRequest } from '../../../types/cashier';
import { Image as ImageIcon } from 'lucide-react';
import toast from 'react-hot-toast';

const ImagingPayment: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const request: ImagingPaymentRequest = {
      ...
      await cashierService.processImagingPayment(request);
      toast.success('Imaging Payment Successful');
      setFormData({ patientEntryId: '', scanType: 'X-Ray', amount: '', paymentMethod: 'CASH' });
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-gray-900">Imaging Payment</h2>
        <p className="text-gray-500 mt-1">Process payment for Radiology / Imaging services</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              className="mt-1 block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-[#9333EA] focus:border-transparent outline-none transition-all"
            >
              <option value="X-Ray">X-Ray (General)</option>
          ...
          <div className="pt-4 flex justify-end">
            ...
            >
              ...
              <span>Confirm Imaging Payment</span>
            </button>
          </div>
        </form>
      </div>
